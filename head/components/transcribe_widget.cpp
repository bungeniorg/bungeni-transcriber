...
*/
    ...
    */
    connect(ui.table, SIGNAL(doubleClicked(QModelIndex)), delegate, SLOT(currentEditing(QModelIndex)));
    connect(ui.table, SIGNAL(clicked(QModelIndex)), delegate, SLOT(display(QModelIndex)));
    //connect(this, SIGNAL(refreshWidget(QModelIndex)), delegate, SLOT(display(QModelIndex)));
  /*
...
//qDebug( "Slot:updateTExt entered" );
    ...
    model->setData(model->index(currentIndex.row(),0), ui.speech->toPlainText());
}
...
//posta->
QString str = "http://localhost/mediawiki/index.php/Special:Speech_from_VLC";
...
/*    if (ba=="success")
    {
        QMessageBox::information(0, "Success", "Post of speeches worked");
    }
    else
    {
        QMessageBox::information(0, "Fail", "Post of speeches failed");
    }
  */
  qDebug() << ba;
  qDebug() << reply->url();
