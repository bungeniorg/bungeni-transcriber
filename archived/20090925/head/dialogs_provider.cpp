...
void DialogsProvider::openTranscripts()
{
    TranscribeWidget::getInstance(p_intf)->openFile();

}

void DialogsProvider::saveTranscripts()
{
    TranscribeWidget::getInstance(p_intf)->saveFile();

}
void DialogsProvider::saveAsTranscripts()
{
    TranscribeWidget::getInstance(p_intf)->saveFileAs();

}

void DialogsProvider::postToBungeni()
{
    ...
    if (keys.size()==0)
    {
          ...
          {
                QMessageBox::warning(0, "Post Cancelled", "Post to Bungeni Cancelled. No Data will be posted.");
          }  
    }
    else
    {      
                TranscribeWidget::getInstance(p_intf)->post();
    }
    
}

void DialogsProvider::getTakes()
{
    TranscribeWidget::getInstance(p_intf)->takes();

}


void DialogsProvider::editPreferences()
{
