...
//    BAR_ADD( HelpMenu( NULL ), qtr( "&Help" ) );
     BAR_ADD( uFileMenu(), qtr( "&File" ) );
     BAR_ADD( uEditMenu(), qtr( "&Edit" ) );
        
}
#undef BAR_ADD
#undef BAR_DADD

QMenu *QVLCMenu::uFileMenu()
{
    QMenu *menu = new QMenu();

    addDPStaticEntry( menu, qtr( "&Open Media File..." ), "",
#ifdef WIN32
...
#endif
/*
addDPStaticEntry( menu, qtr( "&Open Transcript File" ) , "",
        ":/quit", SLOT(  ), "" );
*/
menu->addAction( "Open Transcript File", THEDP, SLOT(openTranscripts()) );

menu->addSeparator();
menu->addAction( "Save Transcript File", THEDP, SLOT(saveTranscripts()) );
menu->addAction( "Save Transcript File As", THEDP, SLOT(saveAsTranscripts()) );
menu->addSeparator();
menu->addAction( "Get Takes", THEDP, SLOT(getTakes()) );
menu->addAction( "Post To Bungeni", THEDP, SLOT(postToBungeni()) );
menu->addSeparator();
...
QMenu *QVLCMenu::uEditMenu()
{
    QMenu *menu = new QMenu();

menu->addAction( "Bungeni Server Settings", THEDP, SLOT(editPreferences()) );
