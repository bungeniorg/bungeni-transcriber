 ...
 * $Id: 62d91be48af147c39475fee317bb63dace6e2515 $
 *
...
void InputManager::sliderUpdate( int new_pos )
{
    
    int i_length = var_GetTime(  p_input , "length" ) / 1000000;
    float temp = (float)(new_pos) / (float)i_length;
//float temp = var_GetFloat(  p_input , "position" );
qDebug() << "position : " << temp;
    if( hasInput() )
        var_SetFloat( p_input, "position", temp);
}
